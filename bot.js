    ...
    bot.sendMessage(chatId, 'תפריט ניהול:', {
        reply_markup: {
            keyboard: [
                ...
                ['📤 שלח הודעה לכולם', '📁 קובץ משתמשים JSON']
            ],
...
bot.on('message', async (msg) => {
    ...
    try {
        if (chatId === adminId.toString()) {
            ...
            } else if (text === '📁 קובץ משתמשים JSON') {
                try {
                    const fileText = fs.readFileSync(USERS_FILE, 'utf-8');
                    await bot.sendMessage(chatId, '📄 תוכן הקובץ:

' + fileText);
                } catch (err) {
                    await bot.sendMessage(chatId, '�?לא ניתן לשלוף את הקובץ. אולי הוא לא קיים עדיין.');
                }
            }
            return;
        }
